...
`validate.containerCommand`|String|"singularity exec -B /.mounts/ -B /scratch3/ $MAVIS_ROOT/bin/mavis.sif"|Command used to enter the mavis container. The mavis module ships only a snakemake wrapper, so mavis and its bundled python helpers are invoked through singularity directly
`validate.singularityTmpDir`|String|"/tmp"|Directory singularity extracts the image into. Must be node-local, an NFS-backed default costs minutes per task
`validate.jobMemory`|Int|18|Memory allocated for this job
`validate.cores`|Int|2|Number of cores allocated for this job
`validate.timeout`|Int|16|Timeout in hours, needed to override imposed limits
`annotate.containerCommand`|String|"singularity exec -B /.mounts/ -B /scratch3/ $MAVIS_ROOT/bin/mavis.sif"|Command used to enter the mavis container. The mavis module ships only a snakemake wrapper, so mavis and its bundled python helpers are invoked through singularity directly
`annotate.singularityTmpDir`|String|"/tmp"|Directory singularity extracts the image into. Must be node-local, an NFS-backed default costs minutes per task
`annotate.jobMemory`|Int|16|Memory allocated for this job
`annotate.cores`|Int|2|Number of cores allocated for this job
`annotate.timeout`|Int|16|Timeout in hours, needed to override imposed limits
